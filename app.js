...
id3_test = {
    parseStream: function (stream, onComplete) {
        ...
        function read (numBytes, callback) {
            stream.read(numBytes, callback);
        };
    ...
    parseFile: function (filePath, callback) {
        var that = this,
            ...
            stream = {
                ...
                read: function(numBytes, callback) {
                    ...
                    fs.read(that.file.fd, buffer, 0, numBytes, that.pointer, function(err, bytesRead) {
                        if(err) throw err;
                        that.pointer += bytesRead;
                        callback(buffer.toString('utf8', 0, bytesRead), buffer);
                    });
                }
            };
        ...
        fs.open(filePath, 'r', function(err, fd) {
            if(err)
            {
                console.log('error: failed to open file ' + filePath);
                throw err;
            }
            file.fd = fd;
            stream.file = file;
            that.parseStream(stream, callback);
        });
    }
};

id3_test.parseFile('./Tip.mp3', function(tag) {
    console.log(tag);
})